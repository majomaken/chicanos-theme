...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize preselected sauce/acompañamiento options (exact match on main label)
    ...
    sauceCards.forEach(card => {
        ...
        // Extraer el nombre principal antes de paréntesis
    ...
    sauceCards.forEach(card => {
        card.addEventListener('click', function() {
            ...
            // Verificar si es una opción preseleccionada (acompañamiento) por coincidencia exacta del nombre principal
            const mainMatchClick = sauceValue.match(/^(.+?)\s*\(/);
            const mainOnlyClick = mainMatchClick ? mainMatchClick[1].trim() : sauceValue;
            const isPreselectedOption = preselectedOptions.includes(mainOnlyClick);
